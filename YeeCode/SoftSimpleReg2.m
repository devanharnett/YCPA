...
for ii=1:10
    start2 = 90; 
    current2 = start2+ii*4; 
    
    start=55; 
    current=start+ii*4; 
    epi{1}(current2:current2+1,current:current+1)= c_eps_0*11.3;
end
...
bc{1}.NumS = 1;
% s(1) sets general program perameters 
bc{1}.s(1).xpos = nx{1}/(4) + 1;
bc{1}.s(1).type = 'ss';
bc{1}.s(1).fct = @PlaneWaveBC;
% mag = -1/c_eta_0;

%bc{1}.s(2).ypos = ny{1}/(4) + 1.5;
bc{1}.s(2).ypos = 3*ny{1}/(4) + 1.5;
bc{1}.s(2).type = 'ss';
...
% a means that it is an absorption boundary
% p is periodic boundary conditions 
% h is magnetic boundary condition
% e is electric field generation
